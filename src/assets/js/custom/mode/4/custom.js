...
Vue.component("order-detailed-dialog",{
	...
	components: {
		'code-components': {
			props:["order"],
			template:
			'<div class="weui_cell weui_vcode table-number" v-if="order.distributionModeId != 2">                                           '+
	...
	methods:{
		...
		pushOrder:function(){
			var that = this;
			if(!this.order.tableNumber){
				this.$dispatch("remindMessage","请扫描桌号二维码或手动输入桌号",3000);
			}else{
				var o = this.order;
		...
		autoPushOrder:function(){
			...
			if(this.show&&this.order.productionStatus==0&&this.order.orderState==2&&this.order.tableNumber){
				this.$dispatch("loadingMessage","正在自动下单",2000);
				this.pushOrder();
			}
		}
	},
